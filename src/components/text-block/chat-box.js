...
export default class ChatBox extends Component {
        ...
        return items.chats.map((it, index) => {
            return (
                <div className={cls(this.initClass + "-list")} key={index}>
                    ...
                    <div className={cls(this.initClass + "-message")}>
                        {it.message}
                    </div>
    ...
    renderAvatars = () => {
        ...
        return <div className="">{avatars} {countMember - 2 > 0 && <Avatar src={it.user.path} role="presentation" >{countMember - 2} + </Avatar>}</div>
    }
    renderNames = () => {
        const { items } = this.props
        let names = []
        ...
        return <div className="">{names} {countMember - 2 > 0 && '...'}</div>
